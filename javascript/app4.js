...
var liWriter = function(album) {
  // var li = document.createElement('li');
  // li.id = albumID;
  // $('albums-and-tracks').append(li);
  // $(albumID).html(albumData);
  // $.each(albumData, function(i, val) {
    
  // });
    var $li = $('<li>');
    var $img = $('<img>');

    $img.attr('src', album.images[2].url);

    $li.attr('data-id', album.id);
    $li.append($img);
    $li.append(album.name);
    $li.append(', '+album.popularity);
    $li.append(', '+album.release_date);
    $('#albums-and-tracks').append($li);
}
    ...
    var getAlbums = $.getJSON('https://api.spotify.com/v1/artists/' + artistID + '/albums').then(function(data) {
            ...
            for (var i = 0; i < albumIDs.length; i++) {
                $.getJSON('https://api.spotify.com/v1/albums/' + albumIDs[i]).then(function(data) {
                        ...
                        //     };
                        // albumData.push(obj); //end object push
                        // $('#albums-and-tracks').append(JSON.stringify(obj));
                    }) //end then function
            } //end for loop
        })
        // }).then(function() {
